...
const propertyQuerySchema = Joi.object({
  ...
  ubicacion: Joi.string().trim().allow('', null),
  ...
  rooms: Joi.number().integer().min(0).max(12),
  habitaciones: Joi.number().integer().min(0).max(12),
  hab: Joi.number().integer().min(0).max(12),
  bedroomsExact: Joi.boolean(),
  habitacionesExactas: Joi.boolean(),
  bathrooms: Joi.number().integer().min(1).max(12),
  banos: Joi.number().integer().min(1).max(12),
  bathroomsExact: Joi.boolean(),
  banosExactos: Joi.boolean(),
  areaMin: Joi.number().integer().min(0),
  area: Joi.number().integer().min(0),
  areaMax: Joi.number().integer().min(0),
  maxArea: Joi.number().integer().min(0),
  strata: Joi.number().integer().min(0).max(10),
  estrato: Joi.number().integer().min(0).max(10),
  furnished: Joi.boolean(),
  petsAllowed: Joi.boolean(),
  utilitiesIncluded: Joi.boolean(),
  parking: Joi.alternatives().try(Joi.boolean(), Joi.number().integer().min(0).max(10)),
  parkingSpots: Joi.number().integer().min(0).max(10),
  elevator: Joi.boolean(),
  balcony: Joi.boolean(),
  security: Joi.boolean(),
  gym: Joi.boolean(),
  gatedCommunity: Joi.boolean(),
  administrationIncluded: Joi.boolean(),
  administracionIncluida: Joi.boolean(),
  ...
  sort: Joi.string()
    .valid('recommended', 'latest', 'rent-asc', 'rent-desc', 'area-desc', 'bedrooms-desc')
    .default('recommended'),
